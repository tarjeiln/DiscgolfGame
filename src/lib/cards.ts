...
const CARDS_THROWSTYLE: Card[] = [
  { id: 'c1',  title: 'Ubestemt',        description: 'Kast dette hullet med motsatt grep.', category: 'ThrowStyle', scope: 'player' },
  { id: 'c4',  title: 'Forehand only',    description: 'Bare forehand på dette hullet.',      category: 'ThrowStyle', scope: 'player'  },
  { id: 'c5',  title: 'Backhand only',    description: 'Bare backhand på dette hullet.',      category: 'ThrowStyle', scope: 'player'  },
  { id: 'c15', title: 'Roller only',      description: 'Alle kast må være rollers.',          category: 'ThrowStyle', scope: 'player'  },
  { id: 'c20', title: 'Maks 2 kastetyper',description: 'Velg to kastetyper (f.eks. backhand & putt). Bare disse er lov.', category: 'ThrowStyle', scope: 'player'  },
  { id: 'c24', title: 'Hyzer only',       description: 'Alle kast skal være hyzer-vinkel.',   category: 'ThrowStyle', scope: 'player'  },
  { id: 'c25', title: 'Anhyzer only',     description: 'Alle kast skal være anhyzer-vinkel.', category: 'ThrowStyle', scope: 'player'  },
  { id: 'c26', title: 'Over hodet',       description: 'Første kast må være thumber eller tomahawk.', category: 'ThrowStyle', scope: 'player'  },
  { id: 'c27', title: 'Ubestemt',  description: 'Alle putter over 5m må gjøres som jump-putt.', category: 'ThrowStyle', scope: 'player'  },
  { id: 'c39', title: 'Ingen run-up',     description: 'Alle kast må gjøres stående uten tilløp.', category: 'ThrowStyle', scope: 'player'  },
  { id: 'c45', title: 'Felles drive',     description: 'Alle kaster samme type kast på første kast (f.eks. backhand hyzer).', category: 'ThrowStyle', scope: 'group'  },
  { id: 'c47', title: 'Sirkelpress',      description: 'Alle kast innenfor 20m må gjøres uten jump-putt.', category: 'ThrowStyle', scope: 'player'  },
  { id: 'c50', title: 'Venstre hånd',     description: 'Spill hele hullet med motsatt hånd.', category: 'ThrowStyle', scope: 'player'  },
  { id: 'c35', title: 'Ubestemt',         description: 'Putt må slippes under midjehøyde.',   category: 'ThrowStyle', scope: 'player'  },
  { id: 'c36', title: 'Ubestemt',         description: 'Putt må slippes over hodet.',         category: 'ThrowStyle', scope: 'player'  },
];
...
const CARDS_SCORING: Card[] = [
  ...
  { id: 'c29', title: 'Korteste drive',   description: 'Den som kaster kortest på utkastet får -1 bonus.', category: 'Scoring', scope: 'group' },
  { id: 'c30', title: 'Lengste drive',    description: 'Den som kaster lengst på utkastet får +1 ekstra.', category: 'Scoring', scope: 'group' },
  { id: 'c31', title: 'Kjedekontroll',    description: 'Treffer du kurvkjetting men misser, får du -1.', category: 'Scoring' },
  { id: 'c38', title: 'Ubestemt',        description: 'Hvis du treffer kurvkant på drive, -1.',       category: 'Scoring' },
  ...
  { id: 'c53', title: 'OB-bonus',         description: 'Alle som unngår OB på hullet får -1.',         category: 'Scoring', scope: 'group' },
  { id: 'c54', title: 'CTP',              description: 'Closest-to-pin på drive får -1.',              category: 'Scoring', scope: 'group' },
  { id: 'c55', title: 'Siste plass',      description: 'Spilleren med dårligst score så langt får -1.', category: 'Scoring', scope: 'group' },
  { id: 'c56', title: 'Beste plass',      description: 'Spilleren med best score så langt får +1.',    category: 'Scoring', scope: 'group' },
  ...
  { id: 'c72', title: 'Felles bonus',     description: 'Klarer hele flighten par eller bedre, alle -1.', category: 'Scoring', scope: 'group' },
];
...
const CARDS_CHALLENGE: Card[] = [
  ...
  { id: 'c32', title: 'Rask runde',       description: 'Alle må kaste innen 15 sekunder fra turen deres starter.', category: 'Challenge', scope: 'group' },
  { id: 'c33', title: 'Strek-linje',      description: 'Alle kast må krysse en tenkt linje (bestem før start).', category: 'Challenge', scope: 'group' },
...
const CARDS_DISCLIMIT: Card[] = [
  ...
  { id: 'c43', title: 'Rimelig drive',    description: 'Alle må bruke den rimeligste disken sin på første kast.', category: 'DiscLimit', scope: 'group' },
  { id: 'c44', title: 'Dyreste disk',     description: 'Alle må bruke den dyreste disken sin på første kast.', category: 'DiscLimit', scope: 'group' },
...
const CARDS_OTHER: Card[] = [
  { id: 'c17', title: 'Stillekast',           description: 'Hele flighten må være stille mens du kaster – bryter noen reglen, +1 på dem.', category: 'Other', scope: 'group' },
  { id: 'c18', title: 'Motsatt tee-rekkefølge', description: 'Den som egentlig skulle kastet sist, starter.', category: 'Other' },
  { id: 'c37', title: 'Tilfeldig rekkefølge', description: 'Trekk lodd om rekkefølgen på utkastet.', category: 'Other', scope: 'group' },
  { id: 'c67', title: 'Kast på nytt',         description: 'Må kaste første kast to ganger, velg beste.', category: 'Other' },
  { id: 'c68', title: 'Dobbel drive',         description: 'Alle spillere kaster to drivere, velg verste.', category: 'Other', scope: 'group' },
];
...
export function buildDeck(opts?: { include?: Card['category'][]; minSize?: number }) {
  const pool = opts?.include?.length ? CARDS.filter(c => opts!.include!.includes(c.category)) : CARDS;

  // splitt i player- og group-kort
  const playerPool = pool.filter(c => c.scope !== 'player'); // default/undefined = player
  const groupPool  = pool.filter(c => c.scope === 'group');

  // bygg spillerstokk
  const target = opts?.minSize ?? playerPool.length;
  const ids: ID[] = [];
  while (ids.length < target) ids.push(...playerPool.map(c => c.id));
  const deck = shuffle(ids.slice(0, target));

  // bygg modifikasjonsstokk
  const modDeck = shuffle(groupPool.map(c => c.id));

  const cards = Object.fromEntries(pool.map(c => [c.id, c])) as Record<ID, Card>;
  return { cards, deck, modDeck };
}
